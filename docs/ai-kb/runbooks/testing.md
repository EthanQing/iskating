...
- UI 布局分别在 1920×1080（侧栏展开）和 1366×768 检查实时训练、历史复盘、训练设置、轨迹详情及系统设置四个分类。
- 缩放窗口和拖动工作区分隔条，确认 12 路 Camera 自动重排且没有横向滚动，视频和固定训练操作仍可见；长运动员名、比赛名及路径不得撑宽表单。
- 训练设置打开、关闭和快速重复开关后，检查 Inspector 尺寸、主操作、键盘焦点和窗口层级；真实 D3D 视频播放时确认来源叠层和 Camera 选中状态仍可见。
- 历史无记录时空状态位于结果区域中央；展开更多筛选和记录详情后仍可查询、分页和调用原有复盘操作。轨迹详情必须有运动员、时间范围和中文关闭操作。
