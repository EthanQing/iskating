...
- 实时检查栏按当前选中运动员匹配身份，不使用画面中第一个目标代替；检测数来自当前机位结果。
- 当前机位结果按 UI 收到时间以 2 秒为过期阈值，主视频停止播放时清空；独立状态定时器每秒检查过期并刷新时钟，不依赖训练计时器。
- 人工绑定窗口使用打开时的检测快照，避免弹窗期间实时结果变化导致绑定到其他 track。
- 二维轨迹展开显示当前运动员数据的快照，不是独立的实时分析窗口。
