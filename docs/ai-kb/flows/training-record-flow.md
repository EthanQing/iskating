...
14. `openSessionVideo()` 优先用 `cameraDefaults/nvrPlaybackTemplate`、session 开始时间、机位 IP 和动作片段窗口生成 NVR RTSP 回放 URL；模板不可用时优先按动作关联的视频资产查找本地文件，再回退到保存的视频引用。离线视频“定位片段”按 `videoClipStartMs` 请求 seek。
