...
- 基于 Windows Qt 客户端、FastAPI/PostgreSQL、DeepStream worker、工具和测试完成 143 个编号项的全量功能/边界盘点，并将客户端重编为“工作台→准备→三种执行模式→任务→复盘→报告→资料/设置”九个目标区域。
- 新增无外部依赖的 `docs/client-flow-prototype.html`，用于演示重编后的导航、实时/单视频/12 路分析分流、任务边界和当前结果/旧评分分层。该原型不代表 Qt 客户端已完成导航重构。
- 统一 README、用户指南和 AI 知识库到当前事实：新主链路是 person/ReID/机位内 track 与条件式二维轨迹/速度，不生成新姿态、3D、自动动作或技术评分。
- 记录已确认的主 participant UUID 轨迹/速度漏存、任务重启无法继续/进度量纲、完整分析校验范围与模型/gallery 非冻结快照等边界，本次仅分析与原型设计，未修改业务代码。

- 完成 F-05 验收补齐：批量连通测试新增地址解析状态、RTSP Open/首帧耗时、失败阶段、稳定错误码及原始 FFmpeg 返回码展示；测试仍在后台进行且不保存结果或影响当前播放。

- 完成 F-29：新增桌面单并发 `AnalysisTaskManager`、任务中心对话框和可持久化 `paused` 状态；本地视频探测/入库移出 UI 线程，完整帧率远端运行在后台轮询。当前进程内可暂停/继续；完整分析暂停只停轮询，重启后只恢复 `paused` 展示而不重建 job 参数，需重新发起。

- 完成 F-25：新增 `speed_metrics`，以轨迹点关联保存每名运动员的瞬时/平滑滑行速度、平滑窗口、单位、算法版本和有效标记；历史轨迹复盘支持按人查看并导出速度曲线。
