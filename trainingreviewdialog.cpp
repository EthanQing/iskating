...
PoseFrameResult poseFrameFromJson(const QString &json)
{
    ...
    frame.cameraId = root.value(QStringLiteral("cameraId")).toInt();
    frame.timestampMs = root.value(QStringLiteral("timestampMs")).toString().toLongLong();
    frame.frameSize = QSizeF(root.value(QStringLiteral("width")).toDouble(),
                             root.value(QStringLiteral("height")).toDouble());
    frame.skeletonType = static_cast<PoseSkeletonType>(root.value(QStringLiteral("skeletonType")).toInt());
    frame.sourceName = root.value(QStringLiteral("sourceName")).toString();

    const QJsonArray instances = root.value(QStringLiteral("instances")).toArray();
    for (const QJsonValue &instanceValue : instances) {
        ...
        instance.box = QRectF(instanceObject.value(QStringLiteral("x")).toDouble(),
                              instanceObject.value(QStringLiteral("y")).toDouble(),
                              instanceObject.value(QStringLiteral("w")).toDouble(),
                              instanceObject.value(QStringLiteral("h")).toDouble());

        const QJsonArray keypoints = instanceObject.value(QStringLiteral("keypoints")).toArray();
        for (const QJsonValue &keypointValue : keypoints) {
            const QJsonObject keypointObject = keypointValue.toObject();
            PoseKeypoint keypoint;
            keypoint.index = keypointObject.value(QStringLiteral("index")).toInt();
            keypoint.name = keypointObject.value(QStringLiteral("name")).toString();
            keypoint.imagePoint = QPointF(keypointObject.value(QStringLiteral("x")).toDouble(),
                                          keypointObject.value(QStringLiteral("y")).toDouble());
            keypoint.point3d = QVector3D(static_cast<float>(keypointObject.value(QStringLiteral("x3")).toDouble()),
                                         static_cast<float>(keypointObject.value(QStringLiteral("y3")).toDouble()),
                                         static_cast<float>(keypointObject.value(QStringLiteral("z3")).toDouble()));
            keypoint.confidence = static_cast<float>(keypointObject.value(QStringLiteral("confidence")).toDouble());
            keypoint.valid = keypointObject.value(QStringLiteral("valid")).toBool();
            keypoint.hasPoint3d = keypointObject.value(QStringLiteral("hasPoint3d")).toBool();
            instance.keypoints.append(keypoint);
        }
...
void TrainingReviewDialog::buildUi()
{
    ...
    m_overlayCheckBox = new QCheckBox(QStringLiteral("姿态叠加"), this);
    ...
    controlLayout->addWidget(m_overlayCheckBox);
    ...
    connect(m_overlayCheckBox, &QCheckBox::toggled, this, [this](bool enabled) {
        if (!enabled || m_currentRow < 0 || m_currentRow >= m_repetitions.size()) {
            clearPoseOverlay();
            return;
        }
        applyPoseOverlay(m_repetitions.at(m_currentRow));
    });
...
void TrainingReviewDialog::applyPoseOverlay(const ActionRepetition &repetition)
{
    if (repetition.keyFramePoseJson.trimmed().isEmpty()) {
        clearPoseOverlay();
        return;
    }
    m_mainVideo->setPoseFrame(poseFrameFromJson(repetition.keyFramePoseJson));
}

void TrainingReviewDialog::clearPoseOverlay()
{
    if (m_mainVideo) {
        m_mainVideo->setPoseFrame({});
    }
}
