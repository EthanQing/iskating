...
实时训练下半区由 Camera Grid 与始终可见的二维滑行轨迹共用一行，轨迹卡片位于右侧，宽度按下半区可用宽度的约 30% 分配并限制在 300～460px（常规桌面约 380～460px），为固定比例的扁长跑道提供更大的展示空间。最大化和全屏固定 4×3；普通窗口优先 4×3，仅在三行高度预算不足且网格自身宽度足够时退回 6×2。Camera Grid 横向扩展并由等权列铺满轨迹卡片之外的实际剩余宽度，不再按高度回缩 Tile 宽度或居中排列。下半区优先使用按 Tile 宽度计算的 16:9 高度；极宽矮窗口仅限制 Tile 高度，保留主视频更大的高度。两区由父容器统一控制高度，与主视频左右对齐，不再提供轨迹展开或折叠入口。轨迹使用 QPainter 绘制深色 Stadium 跑道示意底图，以固定 2.6:1 比例居中适配可用区域，由长直道和两端半圆构成，内圈沿用相同几何；空数据和等待场地标定时也显示；底图不代表官方冰场尺寸，不改变 fieldBounds 或轨迹采样，真实米制坐标沿用线性映射到跑道内侧有效矩形，不吸附赛道。Camera Segment 仅以顶部低权重标记呈现，标题与当前运动员信息由卡片负责，刷新沿用现有链路，不增加定时器。
