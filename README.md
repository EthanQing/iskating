...
实时训练采用主视频、12 路横向视频源带、二维轨迹和右侧运动员检查器。点击视频源切换主视图，右键可控制该路播放；侧栏可折叠为图标导航。主运动员在检查器顶部选择，模型精度、分析帧率和其他训练上下文从“训练设置”展开。训练操作保留开始、暂停、停止和保存，F11/ Esc 用于进入/退出全屏。

本阶段重建应用框架和实时训练页；历史复盘、人员、比赛及系统设置保留既有业务入口，未做完整页面重构。
