...
实时训练下半区由 Camera Grid 与始终可见的二维滑行轨迹共用一行，轨迹卡片位于右侧，宽度随可用空间限制在 340～380px。最大化和全屏固定 4×3；普通窗口优先 4×3，仅在三行高度预算不足且网格自身宽度足够时退回 6×2。Camera Tile 按网格实际宽度和可用高度保持接近 16:9；两区由父容器统一控制高度并沿用主视频的空间预算，不再提供轨迹展开或折叠入口。轨迹使用 QPainter 绘制深色 Stadium 跑道示意底图，以固定 2.6:1 比例居中适配可用区域，由长直道和两端半圆构成，内圈沿用相同几何；空数据和等待场地标定时也显示；底图不代表官方冰场尺寸，不改变 fieldBounds 或轨迹采样，真实米制坐标沿用线性映射到跑道内侧有效矩形，不吸附赛道。Camera Segment 仅以顶部低权重标记呈现，标题与当前运动员信息由卡片负责，刷新沿用现有链路，不增加定时器。
