...
Windows 滑冰训练辅助桌面应用，使用 Qt Widgets + QML / C++20 混合界面。支持多机位视频预览、运动员检测与身份识别、训练记录和历史复盘；完成场地标定后可生成二维轨迹与速度。当前新训练不生成自动动作计数、技术评分或 3D 骨架。
...
- Qt 6.7.3，MSVC 2022 64 位版本，包含 Qt Quick、QML、Quick Widgets 和 Quick Controls 模块。
...
VideoOpenGLWidget 自身使用原生窗口承接视频画面和覆盖层，并保留 WA_DontCreateNativeAncestors；这能避免非原生容器移动时，原生状态标签在顶层 backing store 留下旧位置残影。视频画面和覆盖层的原生窗口应限制在 VideoOpenGLWidget 内，避免将外层页面、分隔区和侧栏提升为原生窗口，增加布局动画的窗口调整开销。尚未创建交换链时，清空画面不初始化图形资源；实际视频帧到达后再创建渲染资源。

MainWindow 的外层框架由 `src/ui/mainwindowchrome.h/.cpp` 管理：32px 自定义标题栏横跨原有工作区，左上角复用侧栏展开／折叠按钮，不重复显示应用图标和标题；普通窗口显示 1px 边框，最大化去掉边框，全屏同时隐藏标题栏。Windows 使用原生非客户区命中测试处理标题栏拖动、双击和 6px（随 DPI 缩放）的八方向缩放，保留系统窗口操作；三个窗口按钮沿用 Qt 最小化、最大化／还原和正常关闭流程。框架容器保持非原生 QWidget，不改动视频祖先属性；现有 Modal Scrim 覆盖包含标题栏的整个窗口，模态期间禁止底层框架拖动。
...
实时训练左侧工作区在宽窗口中使用上下两层布局：上方主视频横向铺满，下方左侧为二维滑行轨迹、右侧为 3 列 × 4 行相机宫格。宽窗口保留 Qt Layout 的 3:2 stretch 分配和 12px 面板间距。左侧可用宽度不足 928px 时，轨迹与预览改为上下排列；不足 556px 时预览改为 2 列 × 6 行。预览保留至少 180×112px、轨迹卡片保留至少 360×250px 的可读区域，内容超过视口时通过左侧独立滚动容器访问，不提高主窗口的最小尺寸；极窄分隔区允许水平滚动。窗口缩放只重排原有视频控件，不重建播放器或重启视频流。跑道仍使用固定 2.6:1 的 Stadium 示意几何居中适配，空数据与等待标定状态保留；底图不代表官方尺寸，fieldBounds、真实米制线性映射、采样与刷新链路不变。标题和当前运动员信息由轨迹卡片负责，不提供展开或折叠入口。
...
左侧三项导航、业务页头和右侧训练工作台使用 QML；QMainWindow 与原生窗口框架、D3D11 主视频和 12 路预览、二维轨迹、历史与系统状态页内容及现有弹窗继续使用 QWidget。各 QML 分区相邻布置，不覆盖原生视频；小窗口下训练内容可滚动，折叠导航保留图标、焦点和工具提示，F11 / Esc 沿用原窗口行为。

`MainWindowPresentation` 仅提供可通知状态和操作请求；`MainWindow` 保留业务校验、播放器与保存流程，并按运动员 ID 同步 QML 选择器与训练设置。`QuickPanelHost` 使用 Qt 6.7 的 context property 注入共享展示对象；加载失败会记录 QML 错误并显示提示。QML 源码和 SVG 随 qrc 嵌入，Release 的 `windeployqt --qmldir src/ui/qml` 部署导入模块，运行时优先加载 EXE 同目录的 `qml`，分发时须保留这些目录。

在上文 x64 Native Tools 命令提示符内，从仓库根运行客户端测试：

```bat
C:\Qt\6.7.3\msvc2022_64\bin\qmake.exe tests\client\client-tests.pro -o Makefile.ClientTests "CONFIG+=release"
nmake /nologo /f Makefile.ClientTests.Release
set PATH=C:\Qt\6.7.3\msvc2022_64\bin;%PATH%
x64\Release\tests\client-tests.exe -platform offscreen
```

新增测试加载三个真实 QML 分区，并检验训练按钮单次请求、禁用行为、导航折叠及按 ID 选择运动员。这些无设备测试不代替真实视频、数据库和完整训练保存的集成验证。可用 `qmllint src/ui/qml/*.qml` 检查 QML；Qt 6.7 静态检查器无法解析宿主注入的 `windowPresentation`，该上下文名称以加载及交互测试验证。
